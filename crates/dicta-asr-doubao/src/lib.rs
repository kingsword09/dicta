    ...
    async fn run_live(
        ...
        loop {
            ...
            let record_task = tokio::task::spawn_blocking({
                ...
                move || dicta_audio::record_default_input_to_wav(&chunk_path, chunk_duration)
            });
...
fn temp_recording_path() -> PathBuf {
    ...
    std::env::temp_dir().join(format!("dicta-doubao-live-{millis}.wav"))
}
...
fn doubao_credential_path(config_dir: PathBuf) -> PathBuf {
    config_dir.join("dicta").join("doubao-credentials.json")
}
