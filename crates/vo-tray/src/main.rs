...
impl AppConfig {
    fn from_env() -> Self {
        ...
        if let Ok(value) = env::var("VO_BIN") {
            if !value.trim().is_empty() {
                config.vo_bin = value;
            }
        }
        if let Ok(value) = env::var("VO_UI_LIVE_ARGS") {
        ...
        config.provider_config = non_empty_env("VO_PROVIDER_CONFIG");
        config.provider_state = non_empty_env("VO_PROVIDER_STATE");
        config.autostart = env::var("VO_UI_AUTOSTART").is_ok_and(|value| value == "1");
        config.native_glass = env::var("VO_UI_NATIVE_GLASS").is_ok_and(|value| value == "1");
        config
    }

    fn vo_command(&self) -> Command {
        let mut command = Command::new(&self.vo_bin);
        if let Some(path) = &self.provider_config {
            command.env("VO_PROVIDER_CONFIG", path);
        }
        if let Some(path) = &self.provider_state {
            command.env("VO_PROVIDER_STATE", path);
        }
        command
    }
}
...
impl TrayApp {
    ...
    fn load_provider_report(&self) -> Result<ProviderListReport> {
        let output = self
            ...
            .output()
            .with_context(|| format!("failed to run `{}`", self.config.vo_bin))?;
    ...
    fn start_live(&mut self) {
        ...
        let mut command = self.config.vo_command();
        command
    ...
    fn stop_live(&mut self) {
        ...
        if let Err(error) = child.kill() {
            eprintln!("vo-tray: failed to stop live worker: {error}");
        }
        match child.wait() {
            Ok(status) => {
                eprintln!("vo-tray: live worker exited with {status}");
    ...
    fn restart_live(&mut self) {
        ...
        if self.live_child.is_some() {
            ...
            eprintln!("vo-tray: live was not running; starting selected provider");
        }
        self.start_live();
    }
    ...
    fn build_menu(&mut self) -> Result<Menu> {
        ...
        menu.append(&PredefinedMenuItem::separator())?;
        let quit = MenuItem::with_id(MenuId::new(MENU_QUIT), "Quit vo", true, None);
        menu.append(&quit)?;
        Ok(menu)
    }
    ...
    fn tooltip(&self) -> String {
        match self.selected_provider() {
            Some(provider) => format!("vo: {provider} ({})", self.status),
            None => format!("vo: no active provider ({})", self.status),
        }
    }
        ...
        match event {
            Event::NewEvents(StartCause::Init) => {
                match create_tray_icon(&mut app) {
                    ...
                    Err(error) => {
                        eprintln!("vo-tray: failed to create tray icon: {error}");
                ...
                match create_panel(_target, panel_proxy.clone(), &app) {
                    ...
                    Err(error) => {
                        eprintln!("vo-tray: failed to create panel UI: {error}");
                    }
                }
            }
...
fn refresh_shell(app: &mut TrayApp, tray_icon: Option<&TrayIcon>, panel: Option<&Panel>) {
    ...
    {
        eprintln!("vo-tray: failed to update tray icon: {error}");
    }
    if let Some(panel) = panel
        && let Err(error) = update_panel(panel, app)
    {
        eprintln!("vo-tray: failed to update panel UI: {error}");
    }
}
...
fn create_panel(
    ...
    #[cfg(target_os = "macos")]
    if native_glass {
        eprintln!("vo-tray: using macOS native glass panel background");
    } else if native_glass_requested {
        let _ = webview.evaluate_script("window.__voNativeGlass = false; document.querySelector('.shell')?.classList.remove('native-glass');");
    }
    #[cfg(not(any(
        target_os = "windows",
        target_os = "macos",
...
fn update_panel(panel: &Panel, app: &TrayApp) -> Result<()> {
    ...
    let script = format!("window.__voUpdate({state});");
    panel
        .webview
...
fn render_panel_html(app: &TrayApp, native_glass: bool) -> Result<String> {
    ...
    Ok(PANEL_HTML
        .replace("__VO_PANEL_CSS__", PANEL_CSS)
        .replace("__VO_PANEL_JS__", PANEL_JS)
        .replace(
            "__VO_NATIVE_GLASS__",
            if native_glass { "true" } else { "false" },
        )
        .replace("__VO_INITIAL_STATE__", &state))
}
