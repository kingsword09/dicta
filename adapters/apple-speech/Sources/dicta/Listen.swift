...
func runListen(
    ...
    // Claim dicta's own TCC identity before touching audio, so the Microphone / Speech
    // / Audio Recording grants attach to dicta rather than the launching terminal. Done
    ...
    // Info.plist) it is a no-op and dicta keeps the terminal's identity.
    // File mode still re-execs because the Speech Recognition grant attaches to dicta
...
private func finalizeSession(
    ...
    if let sessionLog {
        ...
        // Otherwise `dicta --json` run from an interactive shell — where STDIN and STDOUT
...
private func printBanner(
    ...
    let version = Dicta.configuration.version
    print("dicta \(version) — listening on \(channels) (\(langs))")
...
private func printFileBanner(
    ...
    print("dicta \(version) — transcribing \(display) (\(langs))")
    print("")
}
