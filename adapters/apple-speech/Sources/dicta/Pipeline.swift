...
struct Pipeline {
    ...
    private func runChannel(
        ...
        guard let analyzerFormat = await SpeechAnalyzer.bestAvailableAudioFormat(compatibleWith: perLocale.map { $0.transcriber }) else {
            throw DictaError.noCompatibleAudioFormat
        }
        ...
        Task.detached {
            ...
            while true {
                ...
                do {
                    ...
                    emitProgress("dicta: the \(channel.deviceDescription) changed. Following the new default.")
                } catch is CancellationError {
                    break
                } catch {
                    emitProgress("dicta: the \(channel.deviceDescription) changed but the new default could not be opened. Stopping this channel.")
                    break
                }
            }
            for b in inputBuilders { b.finish() }
    ...
    ///     A mid-stream read failure surfaces as a thrown DictaError.inputFileReadFailed
    ...
    private func runFileChannel(
        ...
        do {
            ...
            throw DictaError.inputFileOpenFailed(url: inputURL, underlying: error)
        }
        ...
        // DictaError.inputFileReadFailed; the success path closes the buffer cleanly so
        ...
        let resampler: Task<Void, Error> = Task.detached { [inputURL] in
            ...
            while !Task.isCancelled {
                ...
                do {
                    ...
                    throw DictaError.inputFileReadFailed(url: inputURL, underlying: error)
                }
    ...
    private func ensureSpeechAsset(locale: Locale) async throws {
        ...
        guard isSupported else {
            throw DictaError.unsupportedSpeechLocale(locale, supported: supportedIDs)
        }
    ...
    private func ensureTranslationModel(source: Locale, target: Locale) async throws {
        ...
        case .installed:
            return
        case .supported:
            throw DictaError.translationModelNotInstalled(source: source, target: target)
        case .unsupported:
            throw DictaError.unsupportedTranslationPair(source: source, target: target)
        @unknown default:
...
enum DictaError: Error, CustomStringConvertible {
    ...
    var description: String {
        ...
        case .translationModelNotInstalled(let s, let t):
            ...
            (Run `dicta --doctor` to see translation languages available on this device.)
            """
        ...
        case .unsupportedTranslationPair(let s, let t):
            ...
            Run `dicta --doctor` to see available translation languages.
            """
        ...
        case .unsupportedSpeechLocale(let l, let supported):
            ...
            return """
            SpeechTranscriber does not support locale \(id). Run `dicta --doctor` to see all supported locales.
            """
        }
    }
}
