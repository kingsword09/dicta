        ...
        if voiceProcessing {
            do {
                ...
                FileHandle.standardError.write(Data(
                    "dicta: warning: voice processing unavailable on mic input, continuing without echo cancellation (\(error.localizedDescription))\n".utf8
                ))
            }
        }
...
/// would force a screen-capture grant even though dicta only ever wanted the audio.
final class SpeakerCapture: @unchecked Sendable {
    ...
    private let ioQueue = DispatchQueue(label: "dicta.spk.audio")
    ...
    private func startUnchecked() async throws {
        // Global system-output tap. dicta never plays audio, so there is nothing of
        ...
        let description: [String: Any] = [
            kAudioAggregateDeviceNameKey: "dicta-system-tap",
