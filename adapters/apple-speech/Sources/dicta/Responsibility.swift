...
// terminal's. There is no exec-based equivalent, which is why claiming dicta's own
...
enum Responsibility {
    ...
    private static let guardKey = "DICTA_DISCLAIMED"

    /// Re-launch dicta as its own responsible process so the Microphone / Speech /
    /// Audio Recording prompts attach to dicta rather than the host terminal.
    ///
    ...
    private static func executablePath() -> String? {
        ...
        // Resolve symlinks (Homebrew links dicta into its bin) so the spawned path is
