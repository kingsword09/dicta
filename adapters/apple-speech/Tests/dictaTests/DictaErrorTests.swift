...
@Suite("DictaError messages")
struct DictaErrorTests {
    ...
    @Test func unsupportedLocaleSuggestsRegionalVariants() {
        let err = DictaError.unsupportedSpeechLocale(
    ...
    @Test func translationModelNotInstalledNamesPairAndInstallPath() {
        let err = DictaError.translationModelNotInstalled(
    ...
    @Test func unsupportedTranslationPairPointsToDoctor() {
        let err = DictaError.unsupportedTranslationPair(
            source: Locale(identifier: "en-US"),
    ...
    @Test func inputFileOpenFailedIncludesPathAndUnderlying() {
        let url = URL(fileURLWithPath: "/tmp/dicta-test-missing.wav")
        let underlying = NSError(
            domain: NSCocoaErrorDomain,
            code: NSFileReadNoSuchFileError,
            userInfo: [NSLocalizedDescriptionKey: "No such file"]
        )
        let err = DictaError.inputFileOpenFailed(url: url, underlying: underlying)
        let msg = err.description

        #expect(msg.contains("Could not open"))
        #expect(msg.contains("/tmp/dicta-test-missing.wav"))
    ...
    @Test func inputFileReadFailedNamesCorruptionAndPath() {
        let url = URL(fileURLWithPath: "/tmp/dicta-test-truncated.wav")
        let underlying = NSError(
            domain: NSOSStatusErrorDomain,
            code: -50,
            userInfo: [NSLocalizedDescriptionKey: "I/O error"]
        )
        let err = DictaError.inputFileReadFailed(url: url, underlying: underlying)
        let msg = err.description

        #expect(msg.contains("Read failure"))
        #expect(msg.contains("/tmp/dicta-test-truncated.wav"))
