...
- `--input PATH` transcribes an on-disk audio file (any format `AVAudioFile` can read: wav, m4a, mp3, caf, aiff, …) instead of live mic / speaker. The feeder yields decoded buffers as fast as the file system allows; processing is bounded by what the analyzer and translator can do per chunk, not by wall-clock playback length. File mode runs as a single `.file` channel with `sessionStart = 0`, so `audio.start` / `audio.end` in the JSONL line up with the file's own timecode. EOF on the file closes the analyzer input, which lets `transcriber.results` drain and the channel exit naturally without SIGINT. Mutually exclusive with `--no-mic` / `--no-speaker` / `--voice-processing` / `--select-device` (each combination is rejected with its own `ValidationError`). The TCC re-exec still runs because the Speech Recognition grant attaches to vo regardless of input; the Microphone / Audio Recording grants are simply not exercised in file mode. See `FileSource` in `AudioSource.swift` and `Pipeline.runFileChannel`.
