...
`audio.start` / `audio.end` are offsets in seconds on a **shared session timeline** whose origin (`sessionStart`) is common to both channels, so mic and speaker chunks are directly comparable. Each channel's `SpeechTranscriber.Result.range` (CMTimeRange) is per-stream, so `Pipeline` lifts it by that channel's `channelStart - sessionStart` offset. The `audio` object is **omitted** when the range is invalid or non-finite: vo does not backfill an approximation, mirroring how a nil `confidence` is dropped rather than zero-filled. A consumer that needs a timecode for every chunk (e.g. an SRT writer) reconstructs the missing one from `timestamp` (always present) and neighbouring chunks, the same way it resolves channel overlap. A device-follow rebind keeps the same analyzer, so `channelStart` is recorded once and does not move mid-session. `seq` is monotonic across channels. `src.lang` echoes the BCP-47 form of `--src` (so `--src en` would give `"lang":"en"`, not `"en-US"`).
