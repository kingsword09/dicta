...
export class DictaTranscriptionError extends Error {
  constructor(message, options = {}) {
    super(message);
    this.name = "DictaTranscriptionError";
...
export function transcriptionUrl(apiBase) {
  ...
  if (!base) {
    throw new DictaTranscriptionError("apiBase is required");
  }
...
export function normalizeDictaConfig(config = {}) {
  return {
...
export function createAudioFile(audio, filename) {
  ...
  if (typeof BlobCtor === "undefined" || !(audio instanceof BlobCtor)) {
    throw new DictaTranscriptionError("audio must be a Blob or File");
  }

  const type = audio.type || recommendedRecordingMimeType();
  const name = filename || `dicta-recording.${recommendedRecordingExtension(type)}`;
  if (typeof FileCtor === "undefined") {
    throw new DictaTranscriptionError("File is unavailable in this environment");
  }
...
export async function transcribeAudio(audio, config, options = {}) {
  const normalized = normalizeDictaConfig(config);
  ...
  if (!normalized.model) {
    throw new DictaTranscriptionError("model is required");
  }
  ...
  if (!response.ok) {
    throw new DictaTranscriptionError(`Transcription failed with HTTP ${response.status}`, {
...
export function createDictaAudioRecordedHandler(config, options = {}) {
...
export class DictaMicrophoneRecorder {
  constructor(options = {}) {
  ...
  async start() {
    if (!supportsMicrophoneRecording()) {
      throw new DictaTranscriptionError("This browser does not support microphone recording");
    }
    ...
    this.stopPromise = new Promise((resolve, reject) => {
      ...
      this.recorder.addEventListener("error", (event) => {
        reject(event.error || new DictaTranscriptionError("Microphone recording failed"));
      });
      this.recorder.addEventListener("stop", () => {
        ...
        const filename =
          this.options.filename || `dicta-recording.${recommendedRecordingExtension(type)}`;
...
export class DictaSpeechRecorderElement extends BrowserHTMLElement {
  ...
  async startRecording() {
    try {
      ...
      this.recorder = new DictaMicrophoneRecorder({
        maxSeconds: Number.isFinite(maxSeconds) && maxSeconds > 0 ? maxSeconds : 0,
      });
      await this.recorder.start();
      this.setState("recording");
      this.emit("dicta-recording-start", {});
  ...
  async transcribeRecordedFile(file) {
    this.emit("dicta-audio-recorded", { file });
    this.setState("transcribing");
    this.emit("dicta-transcription-start", { file });
    ...
    this.emit("dicta-transcription", result);
  }

  fail(error) {
    const message = error instanceof Error ? error.message : String(error);
    this.setState("error", message);
    this.emit("dicta-error", { error, message });
  }

  setState(state, text = "") {
    this.state = state;
    this.updateView(text);
    this.emit("dicta-state-change", { state, text });
  }
  ...
  render() {
    this.shadowRoot.innerHTML = `
      <style>
        :host {
          --dicta-bg: oklch(0.985 0.006 205);
          --dicta-ink: oklch(0.2 0.016 230);
          --dicta-muted: oklch(0.46 0.018 230);
          --dicta-line: oklch(0.83 0.012 220);
          --dicta-accent: oklch(0.55 0.12 172);
          --dicta-danger: oklch(0.52 0.16 28);
          display: inline-grid;
          gap: 8px;
          color: var(--dicta-ink);
        ...
        button {
          ...
          gap: 10px;
          border: 1px solid var(--dicta-line);
          border-radius: 8px;
          padding: 0 14px;
          background: var(--dicta-bg);
          color: var(--dicta-ink);
          cursor: pointer;
          font: inherit;
          font-weight: 760;
        ...
        button:hover {
          border-color: color-mix(in oklch, var(--dicta-accent) 58%, var(--dicta-line));
        }

        button:focus-visible {
          outline: 3px solid color-mix(in oklch, var(--dicta-accent) 32%, transparent);
          outline-offset: 2px;
        }
        ...
        [part="indicator"] {
          ...
          height: 10px;
          border-radius: 999px;
          background: var(--dicta-muted);
        }

        :host([data-state="recording"]) [part="indicator"] {
          background: var(--dicta-danger);
          animation: dicta-pulse 1.1s ease-out infinite;
        }

        :host([data-state="transcribing"]) [part="indicator"] {
          background: var(--dicta-accent);
        }

        [part="status"] {
          min-height: 20px;
          color: var(--dicta-muted);
          font-size: 12px;
        }

        :host([data-state="error"]) [part="status"] {
          color: var(--dicta-danger);
        }

        @keyframes dicta-pulse {
...
export function defineDictaSpeechRecorder(name = "dicta-speech-recorder") {
  if (typeof customElements === "undefined") return;
  if (!customElements.get(name)) {
    customElements.define(name, DictaSpeechRecorderElement);
  }
}

defineDictaSpeechRecorder();
