#!/usr/bin/env bash
# Run the Apple Speech native adapter tests when a macOS 26 runtime is available.
set -euo pipefail
...
if [ "$OS_MAJOR" -lt 26 ]; then
    echo ">> skipping Apple Speech adapter tests: macOS 26 runtime required, found $(sw_vers -productVersion)"
    exit 0
fi
